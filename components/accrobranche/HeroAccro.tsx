"use client";

import { motion, useReducedMotion } from "framer-motion";
import ClayButton from "../ui/ClayButton";

export default function HeroAccro() {
  const reduce = useReducedMotion();

  return (
    <section id="hero-accro" className="relative pt-16 bg-[#FFF8CC]">
      <div className="relative h-[68vh] min-h-[460px] overflow-hidden">
        {/* Vidéo bg (placeholder — remplacer par une vidéo accrobranche quand dispo) */}
        <video
          src="/images/hero-header.mp4"
          poster="/images/seccion2.jpg"
          autoPlay
          muted
          loop
          playsInline
          preload="auto"
          aria-label="Accrobranche enfant chez Ludykid"
          className="absolute inset-0 w-full h-full object-cover object-center"
          style={{
            WebkitMaskImage:
              "linear-gradient(to bottom, black 88%, rgba(0,0,0,0.55) 96%, transparent 100%)",
            maskImage:
              "linear-gradient(to bottom, black 88%, rgba(0,0,0,0.55) 96%, transparent 100%)",
          }}
        />
        <div className="absolute inset-0 bg-gradient-to-b from-black/55 via-black/40 to-transparent" />
        <div className="absolute bottom-0 inset-x-0 h-20 sm:h-28 bg-gradient-to-b from-transparent to-[#FFF8CC] pointer-events-none" />

        <div className="absolute inset-0 flex items-center justify-center">
          <div className="text-center px-6 max-w-3xl">
            <motion.h1
              initial={reduce ? false : { opacity: 0, y: 24 }}
              animate={reduce ? undefined : { opacity: 1, y: 0 }}
              transition={{ duration: 0.8, ease: [0.34, 1.56, 0.64, 1] }}
              className="font-fredoka text-4xl md:text-6xl font-extrabold text-white leading-tight mb-5 drop-shadow-xl"
            >
              Accrobranche enfant{" "}
              <span className="inline-block px-3 py-1 rounded-clay bg-lk-yellow text-[#3A2A00] -rotate-2 shadow-clay-yellow">
                au Mans
              </span>
            </motion.h1>

            <motion.p
              initial={reduce ? false : { opacity: 0, y: 16 }}
              animate={reduce ? undefined : { opacity: 1, y: 0 }}
              transition={{ duration: 0.7, delay: 0.15, ease: [0.34, 1.56, 0.64, 1] }}
              className="font-nunito text-white/95 text-base md:text-lg max-w-2xl mx-auto mb-7 drop-shadow-lg leading-relaxed"
            >
              À la recherche d&apos;un parcours d&apos;accrobranche pour votre enfant au Mans ? Offrez-lui une aventure en hauteur qui développe équilibre et confiance en soi chez Ludykid.
            </motion.p>

            <motion.div
              initial={reduce ? false : { opacity: 0, y: 16 }}
              animate={reduce ? undefined : { opacity: 1, y: 0 }}
              transition={{ duration: 0.7, delay: 0.3, ease: [0.34, 1.56, 0.64, 1] }}
              className="flex items-center justify-center gap-3 flex-wrap"
            >
              <ClayButton href="/anniversaire" tone="orange" size="lg">
                Réserver votre anniversaire
              </ClayButton>
              <ClayButton href="/billetterie" tone="purple" size="lg">
                Billetterie
              </ClayButton>
            </motion.div>
          </div>
        </div>
      </div>
    </section>
  );
}
